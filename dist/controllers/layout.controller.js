...
exports.createLayout = (0, catchAsyncErrors_1.catchAsyncError)(async (req, res, next) => {
    try {
        const { type } = req.body;
        const isTypeExits = await layout_model_1.default.findOne({ type });
        if (isTypeExits) {
            return next(new ErrorHandler_1.default(`${type} already exits`, 400));
        }
        if (type === "Banner") {
            const { image, title, subtitle } = req.body;
            const myCloud = await cloudinary_1.default.v2.uploader.upload(image, { folder: "Banner" });
            const Banner = {
                type: "Banner",
                banner: {
                    image: {
